...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const fetchUserData = async (userId: string) => {
    ...
    // All three requests failed with 401 → token invalid/corrupt → redirect to login
    const allUnauthorized =
      profileRes.error?.status === 401 &&
      roleRes.error?.status === 401 &&
      branchRes.error?.status === 401;
    ...
    if (profileRes.error) {
      // network / RLS / missing row → treat as active so user isn't blocked
      setStatus("active");
    } else {
      setStatus((profileRes.data?.status as AccountStatus) ?? "active");
    }
