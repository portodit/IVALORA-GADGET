...
import { CURRENT_VERSION } from "@/data/admin/changelog";
...
export function TopNavbar({ pageTitle, onMobileMenuToggle }: TopNavbarProps) {
  ...
  return (
    <header className="sticky top-0 left-0 right-0 h-16 z-30 flex items-center justify-between px-4 md:px-6 bg-card border-b border-border">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-sm md:text-base font-bold text-foreground leading-tight tracking-wide">
            IVALORA RMS
          </h1>
          <button
            onClick={() => navigate("/admin/release-notes")}
            className="text-[10px] md:text-xs text-muted-foreground hidden sm:flex items-center gap-1 hover:text-foreground transition-colors"
            title="Lihat Release Notes"
          >
            Retail Management System v{CURRENT_VERSION}
          </button>
        </div>
      </div>
