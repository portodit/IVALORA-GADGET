...
import { Menu, X, ChevronDown, ShoppingCart, User, MapPin, ClipboardList, LogOut } from "lucide-react";
...
import { getCart } from "@/pages/customer/shop/CartPage";
import { PromoAnnouncementBar, usePromoBar } from "./PromoAnnouncementBar";
...
interface PublicNavbarProps {
  /** @deprecated — promo bar is now auto-managed */
  topOffset?: string;
  /** Set to true to hide the built-in promo bar (e.g. LandingPage manages its own) */
  hidePromoBanner?: boolean;
}

export function PublicNavbar({ topOffset, hidePromoBanner }: PublicNavbarProps = {}) {
  ...
  const showPromo = !hidePromoBanner && promoBar.visible;
  const computedOffset = topOffset || (showPromo ? "36px" : "0px");

  return (
    <>
      {/* Global promo announcement bar */}
      {!hidePromoBanner && <PromoAnnouncementBar />}
          ...
          <div className="max-w-6xl mx-auto px-5 h-16 flex items-center">
            ...
            <div className="flex items-center gap-1.5 shrink-0 ml-8">
              ...
              <div className="relative hidden md:block" onClick={(e) => e.stopPropagation()}>
                ...
                {langOpen && (
                  <div className="absolute right-0 top-full mt-1.5 bg-popover border border-border rounded-xl shadow-lg overflow-hidden py-1 min-w-[160px]">
              ...
              <div className="relative hidden md:block" onClick={(e) => e.stopPropagation()}>
                ...
                {currOpen && (
                  <div className="absolute right-0 top-full mt-1.5 bg-popover border border-border rounded-xl shadow-lg overflow-hidden py-1 min-w-[120px]">
                    ...
                    {profileOpen && (
                      <div className="absolute right-0 top-full mt-1.5 bg-white border border-border rounded-xl shadow-lg overflow-hidden py-1 min-w-[200px] z-50">
                        ...
                        <button onClick={() => { setProfileOpen(false); navigate("/alamat"); }}
                          ...
                          {lang === "en" ? "My Address" : "Alamat Saya"}
                        </button>
      ...
      {/* Spacer — accounts for promo bar offset if present */}
      <div style={{ height: `calc(64px + ${computedOffset})` }} />
    </>
  );
}
